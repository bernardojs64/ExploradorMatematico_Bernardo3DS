...
const tabsData = [
  { id: 'statistics', name: 'Estatística Descritiva', category: 'Estatística', keywords: ['média', 'moda', 'mediana', 'estatística'] },
  { id: 'frequency', name: 'Tabela de Frequência', category: 'Estatística', keywords: ['frequência', 'tabela', 'dados', 'classes'] },
  { id: 'histogram', name: 'Histogramas', category: 'Estatística', keywords: ['histograma', 'gráfico', 'frequência', 'distribuição'] },
  { id: 'sectors', name: 'Gráfico de Setores', category: 'Estatística', keywords: ['setores', 'pizza', 'gráfico', 'porcentagem'] },
  { id: 'probability', name: 'Probabilidade', category: 'Estatística', keywords: ['probabilidade', 'chance', 'eventos'] },
  { id: 'trigonometry', name: 'Relações Trigonométricas', category: 'Trigonometria', keywords: ['seno', 'cosseno', 'tangente', 'círculo'] },
  { id: 'sine-function', name: 'Função do Seno', category: 'Trigonometria', keywords: ['seno', 'função', 'gráfico', 'onda'] },
  { id: 'fundamental', name: 'Relações Fundamentais', category: 'Trigonometria', keywords: ['identidade', 'fundamental', 'trigonométrica'] },
  { id: 'complex', name: 'Números Complexos', category: 'Trigonometria', keywords: ['complexo', 'imaginário', 'módulo'] },
  { id: 'functions', name: 'Funções e Gráficos', category: 'Funções', keywords: ['função', 'gráfico', 'exponencial', 'logaritmo'] },
  { id: 'progressions', name: 'Progressões (PA e PG)', category: 'Funções', keywords: ['pa', 'pg', 'progressão', 'termo'] },
  ...
  { id: 'graph-builder', name: 'Construtor de Gráficos', category: 'Ferramentas', keywords: ['gráfico', 'plotar', 'coordenadas', 'função'] },
];

let currentTab = 'statistics';
let sidebarOpen = window.innerWidth >= 1024;
let darkMode = true;
let favorites = JSON.parse(localStorage.getItem('mathExplorerFavorites') || '[]');
...
function renderContent(tabId) {
  ...
  switch(tabId) {
    ...
    case 'graph-builder':
      ...
      break;
    default:
      contentArea.innerHTML = '<div class="card"><h2 class="card-title">Em desenvolvimento</h2><p>Esta seção está em desenvolvimento.</p></div>';
  }
}
...
function setupFrequencyTable() {
  document.getElementById('freq-type').addEventListener('change', (e) => {
    const groupedOptions = document.getElementById('grouped-options');
    groupedOptions.classList.toggle('hidden', e.target.value === 'simple');
  });
}
...
function generateFrequencyTable() {
  ...
  const data = input.split(/[,\s]+/).map(n => parseFloat(n.trim())).filter(n => !isNaN(n));
...
function generateSimpleFrequency(data) {
  ...
  const sorted = Object.keys(frequency).map(Number).sort((a, b) => a - b);
  
  let html = '<h3 class="card-subtitle">Tabela de Frequência Simples</h3>';
  html += '<table class="data-table">';
  html += '<thead><tr><th>Valor (xi)</th><th>Frequência (fi)</th><th>Freq. Relativa (%)</th><th>Freq. Acumulada</th></tr></thead>';
  html += '<tbody>';
...
function generateGroupedFrequency(data) {
  ...
  html += '<table class="data-table">';
  html += '<thead><tr><th>Classe</th><th>Ponto Médio</th><th>Frequência</th><th>Freq. Relativa (%)</th><th>Freq. Acumulada</th></tr></thead>';
  html += '<tbody>';
...
function generateHistogram() {
  ...
  const data = input.split(/[,\s]+/).map(n => parseFloat(n.trim())).filter(n => !isNaN(n));
  const numClasses = parseInt(document.getElementById('hist-classes').value);
...
function renderSectorChart() {
  return `
    <div class="card">
      ...
      <div class="input-group">
        <label class="input-label">Rótulos (separados por vírgula):</label>
        <input type="text" id="sector-labels" class="input-field" placeholder="Ex: Matemática, Português, Ciências, História">
      </div>
      
      <div class="input-group">
        <label class="input-label">Valores (separados por vírgula):</label>
        <input type="text" id="sector-values" class="input-field" placeholder="Ex: 30, 25, 20, 25">
      </div>
      ...
      <div id="sector-results" class="result-box hidden">
        ...
        <div id="sector-percentages" style="margin-top: 1.5rem;"></div>
      </div>
    </div>
  `;
}
...
function generateSectorChart() {
  ...
  const values = valuesInput.split(/[,\s]+/).map(n => parseFloat(n.trim())).filter(n => !isNaN(n));
  ...
  percHtml += '<table class="data-table"><thead><tr><th>Categoria</th><th>Valor</th><th>Percentual</th></tr></thead><tbody>';
  ...
  document.getElementById('sector-percentages').innerHTML = percHtml;
}
...
function updateTrigonometry() {
  const angle = parseInt(document.getElementById('trig-angle').value);
...
function updateSineFunction() {
  const a = parseFloat(document.getElementById('sine-a').value);
  const b = parseFloat(document.getElementById('sine-b').value);
  const c = parseFloat(document.getElementById('sine-c').value);
  const d = parseFloat(document.getElementById('sine-d').value);
  ...
  const signC = c >= 0 ? '+' : '';
  const signD = d >= 0 ? '+' : '';
  document.getElementById('sine-formula').textContent = 
    `f(x) = ${a}·sen(${b}·x ${signC} ${c}) ${signD} ${d}`;
}
...
function verifyIdentities() {
  const angle = parseFloat(document.getElementById('fund-angle').value);
  const rad = angle * Math.PI / 180;
...
function plotFunction() {
  const type = document.getElementById('func-type').value;
  const a = parseFloat(document.getElementById('func-a').value);
  const b = parseFloat(document.getElementById('func-b').value);
  const c = document.getElementById('func-c') ? parseFloat(document.getElementById('func-c').value) : 0;
...
function calculateProgression() {
  ...
  const a1 = parseFloat(document.getElementById('prog-a1').value);
  const r = parseFloat(document.getElementById('prog-r').value);
  const n = parseInt(document.getElementById('prog-n').value);
  ...
  if (type === 'pa') {
    ...
    document.getElementById('prog-result').innerHTML = `
      ...
      <p style="margin-top: 1rem;"><strong>Sequência:</strong> ${terms.slice(0, 10).map(t => t.toFixed(1)).join(', ')}${n > 10 ? '...' : ''}</p>
    `;
  } else {
    for (let i = 1; i < n; i++) {
...
function testPrime() {
  const num = parseInt(document.getElementById('prime-test').value);
...
function updatePointsList() {
  ...
  list.innerHTML = graphBuilderPoints.map((p, i) => `
...
function updateGraphBuilder() {
  ...
  // Plot function
  const type = document.getElementById('gb-func-type').value;
  const a = parseFloat(document.getElementById('gb-a').value);
  const b = parseFloat(document.getElementById('gb-b').value);
  const c = document.getElementById('gb-c') ? parseFloat(document.getElementById('gb-c').value) : 0;
  const d = document.getElementById('gb-d') ? parseFloat(document.getElementById('gb-d').value) : 0;
